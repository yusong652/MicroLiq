...
Fig. 4.5 demonstrates that the DEM model simultaneously reproduces three key aspects of undrained behavior. The effective stress path (Fig. 4.5(a)) captures the characteristic dilative response of dense sand: $p'$ initially decreases as the specimen contracts, then increases as dilation develops while approaching the critical state line. The stress-strain relationship (Fig. 4.5(b)) reproduces the strain-hardening behavior, where deviatoric stress $q$ increases monotonically with deviatoric strain $\varepsilon_{q}$, with the shear stiffness matching the experimental data. The liquefaction resistance curve (Fig. 4.5(c)) shows good agreement with experimental trends across the full range of CSR, confirming the model's capability to reproduce the characteristic CSR-$N_L$ relationship. The simultaneous agreement across stiffness, dilatancy, and cyclic liquefaction resistance validates both the micro-parameters (Table 4-1) and the combined servo mechanism described in Section 4.2.2.
