...
Fig. 3.13. Evolution of shear wave velocity during cyclic shear and initial shear wave velocity for different $K_{0}$ values (CSR=0.200)

Fig. 3.13 presents the evolution of shear wave velocity $V_s$ during undrained cyclic shear, calculated at quarter-period intervals. The inset shows the initial shear wave velocity $V_{s0}$ for each $K_0$ state. The $K_0$=1.0 specimen exhibits the highest $V_{s0}$ (approximately 195 m/s), while $K_0$=0.5 and $K_0$=2.0 show lower values (approximately 175 and 178 m/s, respectively). As cyclic loading progresses, all specimens show gradual degradation of $V_s$, with a sharp decrease near liquefaction. This stiffness degradation pattern is similar across different $K_0$ states when normalized by $N_L$.

The higher initial stiffness of the $K_0$=1.0 specimen explains why it requires the least cumulative shear work to reach liquefaction despite needing more cycles: higher stiffness results in smaller strain amplitude per cycle, thus less energy dissipation per cycle. Conversely, the $K_0$=0.5 specimen, with lower initial stiffness but higher liquefaction resistance, accumulates more shear work before liquefaction occurs.
...
Fig. 3.14. Mechanical coordination number evolution in cyclic shear and relationship between cyclic number required for liquefaction and initial mechanical coordination number (CSR=0.250)
