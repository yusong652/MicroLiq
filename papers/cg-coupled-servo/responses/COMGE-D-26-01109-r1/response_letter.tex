...
\medskip
\noindent\textbf{Observed stability in the regimes where the clip
engages.}
In this paper, the scenario above is reached only during cyclic
loading. The servo runs stably through both cyclic regimes in
which $\hat{K}_{11}$ can approach the clip: (i)~cyclic
pre-liquefaction loading, exercised in both the
scalar-$\hat{\mathbf{K}}$ mode under $dH/dt=0$ (the $K_0$ suite
that forms the paper's core) and the full $2\times 2$ mode under
constant-total-axial-stress (the supplemental cyclic validation
runs added for R2.6 below); and (ii)~several cycles of cyclic
post-liquefaction loading, in which the clip becomes the binding
constraint on $\hat{K}_{11}$. The tracking-error ($e_r$, $e_z$)
and specimen-volume traces through these regimes are the subject
of the R2.6 validation below and exhibit no divergence or
high-frequency oscillation on any servo channel.
