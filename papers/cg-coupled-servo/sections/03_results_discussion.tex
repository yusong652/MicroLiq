...
\begin{figure*}[t]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \caption{Deviatoric stress vs. $\pmean$ for three \Kzero{} states. Dashed line: critical-state line $q = M_{cs}\,p^{\prime}$ with $M_{cs} = 0.872$, determined under monotonic loading at HCA-torsional Lode parameter $b = 0.5$ (\cref{fig:csl_determination}). For \Kzero{} = 1.0 the cyclic loading stays close to $b = 0.5$ throughout, so the line is the directly applicable CSL; for \Kzero{} = 0.5 and 2.0 the principal axes rotate during each cycle, sweeping a range of Lode angles, and the line serves as a representative reference rather than the strict locus of critical states visited.}
