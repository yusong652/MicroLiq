...
Figure~\ref{fig:force_disp_sequence} visualizes the complementary evolution of contact force chains and particle displacements. At the initial state, force chains are predominantly aligned axially for \Kzero{} = 0.5 and horizontally for \Kzero{} = 2.0, reflecting the maximum principal stress direction. As cyclic loading progresses, the number of strong contacts decreases and particle displacements grow, indicating progressive degradation of the load-bearing skeleton. Near liquefaction, force chains become sparse and reorient with the rotating principal stress direction. In the post-liquefaction state, the force distributions for the two initially distinct anisotropic states converge toward similar configurations, consistent with the convergence of $\alpha$, \Vs{}, and \Zm{} observed above. This convergence provides microscopic evidence that cyclic shearing drives granular assemblies toward a common fabric configuration regardless of the initial stress state.
...
Figure~\ref{fig:fabric_liq_pair}(a) shows that denser specimens have systematically larger $Z_{m0}$ and higher $N_L$, confirming that coordination number reflects the influence of packing compactness. However, within each density group, $Z_{m0}$ does not correlate monotonically with liquefaction resistance. By contrast, Fig.~\ref{fig:fabric_liq_pair}(b) shows a generally positive correlation between $\alpha_0$ and $N_L$ across both density levels: compression-state anisotropy (larger $\alpha_0$) is associated with higher liquefaction resistance, while extension-state anisotropy (smaller $\alpha_0$) is associated with lower liquefaction resistance. A minor reversal is observed in the lower-density group at extreme extension states (\Kzero{} = 1.5 and 2.0), where the $N_L$ difference is only one cycle; this is likely attributable to the discrete nature of cycle counting, since the strain-based liquefaction criterion can only be evaluated at integer cycle boundaries, introducing an inherent $\pm 1$ cycle uncertainty that is significant when $N_L$ itself is small.
