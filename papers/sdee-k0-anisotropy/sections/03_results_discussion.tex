...
Figure~\ref{fig:fabric_liq_pair}(a) shows that denser specimens have systematically larger $Z_{m0}$ and higher $N_L$. However, because $Z_{m0}$ and relative density are strongly coupled under conventional specimen preparation, this cross-density correlation does not establish an independent role for $Z_{m0}$; it may simply reflect the well-known macroscopic density effect. Within each density group, $Z_{m0}$ does not correlate monotonically with $N_L$, further suggesting that $Z_{m0}$ primarily serves as a proxy for packing compactness in the present dataset. By contrast, $\alpha_0$ varies independently of density through changes in \Kzero{}, making it a genuinely independent microscopic variable in this study. Fig.~\ref{fig:fabric_liq_pair}(b) shows a generally positive correlation between $\alpha_0$ and $N_L$ across both density levels: compression-state anisotropy (larger $\alpha_0$) is associated with higher liquefaction resistance, while extension-state anisotropy (smaller $\alpha_0$) is associated with lower resistance. A minor reversal is observed in the lower-density group at extreme extension states (\Kzero{} = 1.5 and 2.0), where the $N_L$ difference is only one cycle; this is likely attributable to the limited resolution of cycle-based counting, which introduces an inherent uncertainty of approximately one cycle---significant when $N_L$ itself is small.

The three-dimensional view in Fig.~\ref{fig:fabric_liq_pair}(c) reveals the asymmetric roles of the two microscopic parameters. The two relative-density groups are primarily separated along the $Z_{m0}$ axis, whereas the variation in $N_L$ within each group is governed by $\alpha_0$. In other words, $Z_{m0}$ largely mirrors the macroscopic density effect, whereas $\alpha_0$ provides directional fabric information that is inaccessible from macroscopic state variables alone.

The physical interpretation is that, in compression states (\Kzero{} < 1.0), a larger fraction of contact normals is oriented perpendicular to the horizontal cyclic shear direction, which tends to stabilize contacts against shear-induced sliding and slows fabric degradation. In extension states (\Kzero{} > 1.0), the contact normals are more aligned with the shear direction, promoting sliding and force-chain collapse, thereby accelerating pore-pressure accumulation and reducing liquefaction resistance. Isolating the independent contribution of \Zm{} would require specimens with identical density but different coordination numbers---achievable, for example, through varied preparation methods---which remains a direction for future investigation.
