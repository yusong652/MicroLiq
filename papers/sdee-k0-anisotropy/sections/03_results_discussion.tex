...
In normalized coordinates (Fig.~\ref{fig:energy_work}(a,\,b)), both $\ru$ and $W_s$ follow similar growth patterns across the three \Kzero{} states and converge as liquefaction is approached, indicating that the energy accumulation process follows a common progression toward liquefaction regardless of initial stress anisotropy. During the intermediate stage, however, the three states separate in $\ru$: the \Kzero{} = 0.5 specimen consistently exhibits the lowest $\ru$, the \Kzero{} = 2.0 specimen the highest, and the \Kzero{} = 1.0 specimen falls in between.

The normalized representation, however, obscures the actual energy input rate per cycle because it compresses different numbers of cycles onto the same horizontal axis. When $W_s$ is plotted against the actual cycle count $N_c$ (Fig.~\ref{fig:energy_work}(c)), the \Kzero{} = 2.0 specimen accumulates energy fastest, \Kzero{} = 1.0 is intermediate, and \Kzero{} = 0.5 accumulates energy most slowly. This indicates that the key difference among anisotropic states lies in how rapidly energy is input per cycle, rather than in the total energy required to trigger liquefaction.
