...
df1 = pd.read_csv("monotonic_dense/shear.csv",header=0)
df2 = pd.read_csv("Nakata_gamma_q_dense.csv", header=None)
strains_shear_xz = df1["shear_strain_x"] * 100.0
strains_dev = np.sqrt(4/3) * strains_shear_xz
stresses_shear = df1["shear_stress_x"] / 1000.
stresses_x = df1["stress_x"] / 1.0e3
stresses_y = df1["stress_y"] / 1.0e3
stresses_lat = (stresses_x + stresses_y) / 2.0
stresses_ini = stresses_lat[0]
stresses_u = - stresses_lat + stresses_ini
stresses_z = df1["stress_z"] / 1000.0
stresses_1 = (stresses_z + stresses_x) / 2.0 + np.sqrt(((stresses_z - stresses_x)/2.0)**2 + stresses_shear**2)
stresses_2 = stresses_y
stresses_3 = (stresses_z + stresses_x) / 2.0 - np.sqrt(((stresses_z - stresses_x)/2.0)**2 + stresses_shear**2)
stresses_p = (stresses_x + stresses_y + stresses_z) / 3.0
